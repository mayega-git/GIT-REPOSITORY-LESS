...
def less_generate_private_key(k, n, q, seed):
    """
    Génère une clé privée supplémentaire différente de l'originale.

    :param k: Nombre de lignes de la matrice génératrice.
    :param n: Nombre de colonnes de la matrice génératrice.
    :param q: Taille du champ fini Fq.
    :param seed: Graine utilisée pour la génération des clés.
    :return: Nouvelle clé privée (private_key).
    """
    np.random.seed(int(seed) + 1)  # Modification pour garantir une clé différente
    new_private_key = np.random.randint(0, q, size=(k, n))
...
# Générer la signature avec la clé originale
signature = less_sign(private_key, message, salt, n, q, t)
print("Signature générée avec la clé originale :")
print(signature)

# Générer une nouvelle clé privée différente
new_private_key = less_generate_private_key(k, n, q, seed)
print("\nNouvelle clé privée :")
print(new_private_key)

# Signer le message avec la nouvelle clé privée
new_signature = less_sign(new_private_key, message, salt, n, q, t)
print("\nSignature générée avec la nouvelle clé privée :")
print(new_signature)
...
def less_verify(public_key, signature, message, n, q):
    ...
    return recalculated_digest == digest

# Vérification des signatures
is_valid_original = less_verify(public_key, signature, message, n, q)
print("\nLa signature avec la clé originale est valide :", is_valid_original)

is_valid_new = less_verify(public_key, new_signature, message, n, q)
print("La signature avec la nouvelle clé privée est valide :", is_valid_new)
